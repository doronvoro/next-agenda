...
export default function TaskTrackingPage() {
  ...
  const getStatusBadge = (status: TaskStatus) => {
    switch (status) {
      case 'pending':
        return <Badge variant="secondary">{t("statusLabels.pending")}</Badge>;
      case 'in_progress':
        return <Badge variant="default">{t("statusLabels.in_progress")}</Badge>;
      case 'completed':
        return <Badge variant="default" className="bg-green-100 text-green-800">{t("statusLabels.completed")}</Badge>;
      case 'overdue':
        return <Badge variant="destructive">{t("statusLabels.overdue")}</Badge>;
    }
  };
  ...
  const getPriorityBadge = (priority: Task['priority']) => {
    switch (priority) {
      case 'high':
        return <Badge variant="destructive">{t("priorityLabels.high")}</Badge>;
      case 'medium':
        return <Badge variant="secondary">{t("priorityLabels.medium")}</Badge>;
      case 'low':
        return <Badge variant="outline">{t("priorityLabels.low")}</Badge>;
    }
  };
